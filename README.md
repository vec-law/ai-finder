...
2. Rozszerza zapytanie o synonimy przez LLM
3. Oblicza embedding rozszerzonego zapytania
4. Wyszukuje top-N dokumentów przez cosine similarity (`pgvector`)
5. Generuje odpowiedź przez LLM na podstawie pobranych dokumentów
...
2. Expands the query with synonyms via LLM
3. Computes the embedding of the expanded query
4. Retrieves top-N documents by cosine similarity (`pgvector`)
5. Generates a response via LLM based on the retrieved documents
...
# Wyszukiwanie z LLM / Search with LLM
...
rag-searcher/
├── chat.py            # czat z LLM
├── semantic_search.py # wyszukiwanie embeddingowe
